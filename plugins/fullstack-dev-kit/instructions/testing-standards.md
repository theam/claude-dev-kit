...
- **Behavioral tests** — every behavioral change gets a test that fails without it, **when the project has a test framework**. The standard is simple: **the suite must be able to catch a revert of this change** — a bug fix needs a regression test that fails when the fix is reverted; a test that passes regardless of the change asserts nothing. If there is no test framework and the user hasn't asked for one, recommend it — don't force it.
