...
**Quality gates are auto-detected at runtime, not persisted here.** The coverage/e2e gates adapt to what the repo has on each run (see `instructions/testing-standards.md`) — do not detect-and-store them (a project's test setup changes over time). Only if the team wants to *force* a policy do they hand-add an optional `gates` block (`coverage`/`e2e`: `auto` default · `required` · `off`); `dev-kit-setup` never writes it.
