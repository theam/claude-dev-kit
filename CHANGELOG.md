...
- **Codex support (experimental).** The kit now targets OpenAI Codex (CLI + VS Code) alongside Claude Code.
  ...
  - **Native Codex plugin:** the repo doubles as a **Codex marketplace** (`.agents/plugins/marketplace.json` + `plugins/fullstack-dev-kit/` with `.codex-plugin/plugin.json`), so Codex installs the kit the same way it installs any plugin: `codex plugin marketplace add theam/claude-dev-kit` → `codex plugin add fullstack-dev-kit@claude-dev-kit`. The plugin's `skills/` are generated from the canonical top-level `skills/` by `scripts/build-codex-plugin.mjs` — **one source of truth** (host-neutral skill bodies serve both hosts).
  - **Unified installer:** the `create-dev-kit` wizard installs natively for **whichever of Claude Code / Codex it detects** (both if both). For Codex it runs the marketplace/plugin add, **registers the MCP your tracker choice implies** (Jira → Atlassian, Linear → Linear; GitHub/Azure use their CLIs), and schedules the telemetry sweep.
  - **Validation status:** **verified against real `codex-cli 0.147`** — `marketplace add` + `plugin add` install *and enable* the plugin (manifests accepted), and the telemetry parser + sweep are confirmed on a real `~/.codex/sessions` rollout (cumulative `token_count` → correct split incl. `cache_write_input_tokens`; baseline/no-backfill, stale-only, kit-gate, watermark, idempotent). Still yours to confirm in-app: skill invocation and MCP OAuth. Session hooks are feature-flagged on the desktop app, so the sweep is the telemetry path (`codex/hooks.json` remains for builds where hooks are enabled).
