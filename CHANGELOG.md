...
### Fixed
- **Never act on (or claim to post to) a closed/stale PR.** A user saw the agent send changes and report posting them to a PR that was already closed — it had carried a PR number over from earlier in the session and acted on it without checking. Added a PR-state guard: `fix-pr`/`pr-fixer` now **resolve the PR from the current branch and require `state == OPEN`** before gathering findings, pushing, commenting, or resolving threads — on a `CLOSED`/`MERGED` PR they stop and offer the real options (reopen / open a fresh PR / point at the right one) instead of a false "posted" report. `create-pr` now checks for an existing open PR on the branch (updates it rather than duplicating), refuses to reuse a closed PR's number, and reports **only the URL the create call actually returned**. `coding-agent` and the portable `work-story` ship step reinforced. Rule throughout: never trust a PR number from memory — re-resolve from the branch. Kit → **0.19.14**.
...
### Added
- **`plan-definition` — a guided discovery/definition phase upstream of `plan-backlog` (#73).** When an idea is still a rough spark (even a one-liner), this new step *defines the problem* before any backlog exists: a guided, Socratic pass that frames the problem (users, outcome, why-now, constraints, success metrics, risks, non-goals) with proposed answers to confirm/adjust, explores **2–4 solution directions** with trade-offs, and produces an approved **product definition** — a definition doc, **not tickets**. On hosts with artifacts it renders as a navigable artifact (reusing #74's doctrine); Markdown elsewhere. The definition then feeds `plan-backlog`, whose framing step is **lighter** when a definition is handed in — but only when the definition is genuinely **grounded**, not merely well-formed. Ships as the portable `plan-definition` skill plus a Claude `/plan-definition` command and `plan-definer` agent — same trio shape as `plan-backlog`. **Grounding & provenance model:** because a one-liner is a valid input, most content is generated — so the skill separates *generating options the PO chooses* (the job) from *asserting unverifiable facts as established* (fabrication), marks every claim's provenance (`[PO]`/`[spark]`/`[proposed]`/`[unverified]`), makes `risks/unknowns` a must-have, allows any must-have to be "not established" rather than invented, and adds a "what does this rest on that nobody verified?" question to the approval gate. Facilitate-never-decide, approval-gated. Full flow: **define → backlog → ticket → PR → follow-ups**. From @santielizondo's idea; hardened via his dogfooding review on #79. Kit → **0.19.15**.
