...
- **Codex support (experimental).** The kit now targets OpenAI Codex (CLI + VS Code) alongside Claude Code.
  ...
  - **Validation status:** verified against a **real `codex-cli 0.147` rollout** — the token parser (cumulative `token_count` → correct split incl. `cache_write_input_tokens`) and the sweep's selection logic (baseline/no-backfill, stale-only, kit-marker gate, watermark, idempotent dedup). Still to verify in-app: skill loading from `~/.codex/skills/` and the orchestrator's Codex-native form. Session hooks are feature-flagged on the current desktop app, which is why the sweep is the primary delivery.
